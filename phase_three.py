"""
Third training phase for the DQN robot.
7x7 grid with border obstacles and goals in all free positions.
Win/lose conditions:
- Win: collect all goals
- Lose: 6 collisions or moves exhausted
...
MAX_MOVES = 100
MAX_COLLISIONS = 6
MODEL_FILE_INPUT = "robot_phase_two_obstacles_model.pth"
MODEL_FILE_OUTPUT = "robot_phase_three_model.pth"
BUFFER_FILE = "robot_phase_three_buffer.pkl"
...
def run_simulation(agent, show_graphics=False):
    ...
    obstacles = create_border_obstacles(GRID_WIDTH_ADVANCED, GRID_HEIGHT_ADVANCED)
    ...
    if show_graphics:
        ...
        plt.ion()
        draw_simulation(robot, goals, obstacles, ax, "Phase Three - Training", current_moves=0)
    ...
    while not victory and not defeat and step_count < MAX_MOVES:
        ...
        if show_graphics:
            draw_simulation(robot, goals, obstacles, ax, "Phase Three - Training", current_moves=step_count)
...
def main():
    """
    Main function that runs the third phase training.
    """
    # Read total simulation count from previous runs
    try:
        with open("simulation_count_phase_three.txt", "r") as f:
    ...
    if agent.load_model(MODEL_FILE_OUTPUT):
        print("Phase three model loaded.")
        ...
        print("Starting phase three! Loaded model from previous phase.")
    ...
    print("=== PHASE THREE - Advanced Training ===")
    ...
    # Final statistics
    print("\n" + "="*50)
    print("PHASE THREE FINAL RESULTS:")
    ...
    with open("simulation_count_phase_three.txt", "w") as f:
