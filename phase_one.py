"""
First training phase for the DQN robot.
5x5 grid with border obstacles and goals in all free positions.
Win/lose conditions:
- Win: collect all goals
- Lose: 4 collisions or moves exhausted
...
MAX_MOVES = 35
MAX_COLLISIONS = 4
MODEL_FILE = "robot_phase_one_model.pth"
...
def run_simulation(agent, show_graphics=False):
    ...
    obstacles = create_border_obstacles(GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER)
    ...
    if show_graphics:
        ...
        plt.ion()
        draw_simulation(robot, goals, obstacles, ax, "Phase One - Training", current_moves=0)
    ...
    while not victory and not defeat and step_count < MAX_MOVES:
        ...
        if show_graphics:
            draw_simulation(robot, goals, obstacles, ax, "Phase One - Training", current_moves=step_count)
...
def main():
    """
    Main function that runs the first phase training.
    """
    # Read total simulation count from previous runs
    try:
        with open("simulation_count_phase_one.txt", "r") as f:
    ...
    if agent.load_model(MODEL_FILE):
        ...
        print("Hello! Let's continue phase one training!")
        NUM_SIMULATIONS = int(input("How many more simulations do you want to run? "))
        
    else:
        print("Hello! Let's start phase one training.")
    ...
    with open("simulation_count_phase_one.txt", "w") as f:
