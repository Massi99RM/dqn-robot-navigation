"""
Second training phase for the DQN robot.
6x6 grid with border obstacles and goals in all free positions.
Win/lose conditions:
...
from grid import (
    ...
    create_border_obstacles, create_all_goals, spawn_at_center
)

# Simulation parameters
MAX_MOVES = 65
MAX_COLLISIONS = 5
MODEL_FILE_INPUT = "robot_phase_one_obstacles_model.pth"
MODEL_FILE_OUTPUT = "robot_phase_two_model.pth"
BUFFER_FILE = "robot_phase_two_buffer.pkl"
...
def run_simulation(agent, show_graphics=False):
    ...
    obstacles = create_border_obstacles(GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE)
    ...
    if show_graphics:
        ...
        plt.ion()
        draw_simulation(robot, goals, obstacles, ax, "Phase Two - Training", current_moves=0)
    ...
    while not victory and not defeat and step_count < MAX_MOVES:
        ...
        if show_graphics:
            draw_simulation(robot, goals, obstacles, ax, "Phase Two - Training", current_moves=step_count)
...
def main():
    """
    Main function that runs the second phase training.
    """
    # Read total simulation count from previous runs
    try:
        with open("simulation_count_phase_two.txt", "r") as f:
    ...
    if agent.load_model(MODEL_FILE_OUTPUT):
        ...
        print("Starting phase two! Loaded model from previous phase.")
    ...
    print("=== PHASE TWO - Intermediate Training ===")
    ...
    with open("simulation_count_phase_two.txt", "w") as f:
