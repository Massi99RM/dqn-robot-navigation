"""
Second training phase with obstacles for the DQN robot.
6x6 grid with border obstacles, four internal obstacles, and goals in all free positions.
Win/lose conditions:
- Win: collect all goals
- Lose: 7 collisions or moves exhausted
...
from grid import (
    GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE, create_intermediate_grid, draw_obstacles,
    create_phase_two_obstacles, create_all_goals, spawn_at_center
)

# Simulation parameters
MAX_MOVES = 85
MAX_COLLISIONS = 7
MODEL_FILE_INPUT = "robot_phase_two_model.pth"
MODEL_FILE_OUTPUT = "robot_phase_two_obstacles_model.pth"
BUFFER_FILE = "robot_phase_two_obstacles_buffer.pkl"
...
def run_simulation(agent, show_graphics=False):
    ...
    obstacles = create_phase_two_obstacles(GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE)
    goals = create_all_goals(GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE, obstacles)
    robot = Robot(GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE)
    
    # Position robot at grid center
    initial_pos = spawn_at_center(GRID_WIDTH_INTERMEDIATE, GRID_HEIGHT_INTERMEDIATE, obstacles)
    ...
    fig, ax = None, None
    if show_graphics:
        fig, ax = create_intermediate_grid()
        plt.ion()
        draw_simulation(robot, goals, obstacles, ax, "Phase Two with Obstacles - Training", current_moves=0)
    ...
    while not victory and not defeat and step_count < MAX_MOVES:
        ...
        if show_graphics:
            draw_simulation(robot, goals, obstacles, ax, "Phase Two with Obstacles - Training", current_moves=step_count)
    ...
    if show_graphics:
        ...
        # Final message
        if victory:
            ax.text(GRID_WIDTH_INTERMEDIATE//2, -1, "VICTORY!", ha='center', va='center', 
                   fontsize=16, color='green', weight='bold')
        else:
            reason = "Too many collisions" if robot.collisions >= MAX_COLLISIONS else "Out of moves!"
            ax.text(GRID_WIDTH_INTERMEDIATE//2, -1, f"DEFEAT: {reason}", ha='center', va='center',
...
def main():
    """
    Main function that runs the second phase with obstacles training.
    """
    # Read total simulation count from previous runs
    try:
        with open("simulation_count_phase_two_obstacles.txt", "r") as f:
    ...
    if agent.load_model(MODEL_FILE_OUTPUT):
        print("Phase two with obstacles model loaded.")
        ...
        print("Starting phase two with obstacles! Loaded model from previous phase.")
    ...
    print("=== PHASE TWO WITH OBSTACLES - Intermediate Training ===")
    print(f"Grid: {GRID_WIDTH_INTERMEDIATE}x{GRID_HEIGHT_INTERMEDIATE}")
    ...
    # Final statistics
    print("\n" + "="*50)
    print("PHASE TWO WITH OBSTACLES FINAL RESULTS:")
    ...
    with open("simulation_count_phase_two_obstacles.txt", "w") as f:
