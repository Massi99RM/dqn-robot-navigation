"""
First training phase with obstacles for the DQN robot.
5x5 grid with border obstacles, two internal obstacles, and goals in all free positions.
Win/lose conditions:
- Win: collect all goals
- Lose: 5 collisions or moves exhausted
...
from grid import (
    GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER, create_beginner_grid, draw_obstacles,
    create_phase_one_obstacles, create_all_goals, spawn_at_center
)

# Simulation parameters
MAX_MOVES = 50
MAX_COLLISIONS = 5
MODEL_FILE_INPUT = "robot_phase_one_model.pth"
MODEL_FILE_OUTPUT = "robot_phase_one_obstacles_model.pth"
BUFFER_FILE = "robot_phase_one_obstacles_buffer.pkl"
...
def run_simulation(agent, show_graphics=False):
    ...
    obstacles = create_phase_one_obstacles(GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER)
    goals = create_all_goals(GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER, obstacles)
    robot = Robot(GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER)
    
    # Position robot at grid center
    initial_pos = spawn_at_center(GRID_WIDTH_BEGINNER, GRID_HEIGHT_BEGINNER, obstacles)
    ...
    fig, ax = None, None
    if show_graphics:
        fig, ax = create_beginner_grid()
        plt.ion()
        draw_simulation(robot, goals, obstacles, ax, "Phase One with Obstacles - Training", current_moves=0)
    ...
    while not victory and not defeat and step_count < MAX_MOVES:
        ...
        if show_graphics:
            draw_simulation(robot, goals, obstacles, ax, "Phase One with Obstacles - Training", current_moves=step_count)
    ...
    if show_graphics:
        ...
        # Final message
        if victory:
            ax.text(GRID_WIDTH_BEGINNER//2, -1, "VICTORY!", ha='center', va='center', 
                   fontsize=16, color='green', weight='bold')
        else:
            reason = "Too many collisions" if robot.collisions >= MAX_COLLISIONS else "Out of moves!"
            ax.text(GRID_WIDTH_BEGINNER//2, -1, f"DEFEAT: {reason}", ha='center', va='center',
...
def main():
    """
    Main function that runs the first phase with obstacles training.
    """
    # Read total simulation count from previous runs
    try:
        with open("simulation_count_phase_one_obstacles.txt", "r") as f:
    ...
    if agent.load_model(MODEL_FILE_OUTPUT):
        print("Phase one with obstacles model loaded.")
        ...
        print("Starting phase one with obstacles! Loaded model from previous phase.")
    ...
    print("=== PHASE ONE WITH OBSTACLES - Base Training ===")
    print(f"Grid: {GRID_WIDTH_BEGINNER}x{GRID_HEIGHT_BEGINNER}")
    ...
    # Final statistics
    print("\n" + "="*50)
    print("PHASE ONE WITH OBSTACLES FINAL RESULTS:")
    ...
    with open("simulation_count_phase_one_obstacles.txt", "w") as f:
